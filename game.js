...
04 00030240`;
  ...
  var Play = class {
    ...
    update(dt) {
      ...
      if (this.dist >= (this.section + 1) * this.sectionLength) {
        this.section++;
        this.nextSection();
      }
      if (this.bgPos > this.g.h) {
    ...
    spawn() {
      if (this.gameOver || this.boss)
        return;
      let level = ~~(this.section * 1.1);
      level = level > 10 ? 10 : level;
      const baseSpawnTime = 150 - this.section * 3;
      const nextSpawn = this.g.H.rnd(baseSpawnTime, baseSpawnTime + 80);
      this.g.addEvent({
        t: nextSpawn,
        cb: () => {
          let baddie = this.g.H.rndArray(this.baddies);
          if (Math.random() > Math.max(0.6, 0.75 - this.section * 0.02)) {
            this.g.spawn("Cactus", { p: this });
          }
          this.g.spawn(baddie, { p: this });
          if (Math.random() > Math.max(0.5, 0.7 - this.section * 0.05)) {
    ...
    nextSection() {
      this.g.addText("SECTION " + (this.section + 1), 0, false, 200, 3, 5);
      this.g.sfx("powerup");
      if (this.section < this.allBaddies.length) {
        this.addBaddie();
      }
      if (this.section >= this.allBaddies.length + 2) {
        this.bossTime();
      }
    }
  };
